...
func TestOptimization_NoMemoryLeak_SustainedLoad(t *testing.T) {
	...
	// Should allocate less than 200 KB per request on average (no leak, bounded alloc)
	if allocPerReq > 200*1024 {
		t.Errorf("excessive allocation: %.0f bytes/req (expected <200KB)", allocPerReq)
	}
}
