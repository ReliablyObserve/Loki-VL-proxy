...
func TestPatternsSnapshotApply_IsAppendOnlyAndLongLivedInCache(t *testing.T) {
	...
	applied, patternApplied := p.applyPatternsSnapshot(patternsSnapshot{
		Version:         1,
	...
	if patternApplied != 1 {
		t.Fatalf("expected first snapshot apply to add one pattern, got %d", patternApplied)
	}
	...
	if patternApplied != 0 {
		t.Fatalf("expected older snapshot update to apply zero patterns, got %d", patternApplied)
	}
	...
	if patternApplied != 1 {
		t.Fatalf("expected newer snapshot update to apply one pattern, got %d", patternApplied)
	}
	...
	if patternApplied != 1 {
		t.Fatalf("expected second snapshot append to apply one pattern, got %d", patternApplied)
	}
