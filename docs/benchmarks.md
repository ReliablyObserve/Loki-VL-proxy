...
**Top allocation hot paths identified from pprof (heavy c=100 run):**

| Hot path | Alloc | % of total | Optimization target |
|----------|-------|-----------|---------------------|
| `encoding/json` decode (`objectInterface`, `mapEncoder`) | ~82 GB | 27–37% | Stream decoder; avoid full unmarshal |
| `translateStatsResponseLabelsWithContext` | ~81 GB | 27% | Reuse intermediate map; selective JSON parse |
| `parseStreamLabels` + `splitLabelPairs` | ~7–23 GB | 10–15% | `sync.Pool` for slice; cache by `_stream` value |
| `bytes.growSlice` (response buffering) | ~18–32 GB | 11–32% | `Buffer.Grow()` pre-allocation |
| `encoding/json` string unquote | ~3–9 GB | 3–5% | Reduced by caching parsed label values |
| `net/textproto` header processing | ~3–8 GB | 1–4% | Cache header lookups per-request |
| `patternLineTokenizer.Tokenize` | ~6.6 GB | 2% | `sync.Pool` for tokenizer slices |
